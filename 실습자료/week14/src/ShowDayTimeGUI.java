...
public class ShowDayTimeGUI extends JFrame {
	/** SerialVersionUID **/
	...
	public ShowDayTimeGUI(String txt1, String txt2) {
		super("다이어리");
	...
	public void dbShowDayTime(String selectedDay, String selectedTime) { //데이터 조회 - 날짜, 시간별 검색
		try { //DB연결
			...
			int len = 0;
			rs = stmt.executeQuery("select count(*) from recordtable where day='"+selectedDay+"' and time='"+selectedTime+"';");
			while (rs.next()) { //끝까지 읽어들이기
				len = Integer.parseInt(rs.getString("count(*)"));
			}
			
			rs = stmt.executeQuery("select * from recordtable where day='"+selectedDay+"' and time='"+selectedTime+"';");
