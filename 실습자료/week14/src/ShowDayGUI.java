...
public class ShowDayGUI extends JFrame {
	/** SerialVersionUID **/
	...
	public void dbShowDay(String selectedDay) { //데이터 조회 - 날짜별 검색
		try { //DB연결
			...
			int len = 0;
			rs = stmt.executeQuery("select count(*) from recordtable where day='"+selectedDay+"';");
			while (rs.next()) { //끝까지 읽어들이기
				len = Integer.parseInt(rs.getString("count(*)"));
			}
			
			rs = stmt.executeQuery("select * from recordtable where day='"+selectedDay+"';");
